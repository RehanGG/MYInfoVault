...
export default function ListTicketsPage() {
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(query(collection(firestore, 'tickets'), where('userId', '==', userId), orderBy("ticketTime", "desc")), querySnapshot => {
            const tmpTickets = [];
                                    ...
                                    return (
                                        <tr key={ticket.docId}>
                                            ...
                                            <td className="flex flex-col px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                                                ...
                                                <button onClick={() => {handleDelete(ticket.docId)}} className="text-red-600 hover:text-red-900">
                                                    Delete
                                                </button>
                                            </td>
    ...
    return (
        <>
            <h3 className="text-gray-700 text-3xl font-semibold mb-8">Your Tickets</h3>
            {loading && <Loader/>}
